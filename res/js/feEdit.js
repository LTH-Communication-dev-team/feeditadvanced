...
//			,dropAllowed: 'feEditAdvanced-dropzone'
	...
	pushContentUpdate: function(json) {
		...
		//table = 'tt_content';
                var tableuid = $(json.content).attr('id');
                var tableuidArray = tableuid.split(':');
                var table = tableuidArray[0];
...
TYPO3.FeEdit.EditAction = Ext.extend(TYPO3.FeEdit.EditPanelAction, {
	...
	trigger: function() {
		...
		if (this.parent && this.parent.getTableName() == 'pages') {
			...
			label = resources.editContentElement;
		}
			// attach new editPanel
		FrontendEditing.editWindow.setEditPanel(this.parent);
...
function copyAction(json)
{
    ...
    //$('.feEditAdvanced-editButton.cutAction').after('<input type="button" onclick="pasteAction('+this.parent.id+');return false;" class="feEditAdvanced-editButton pasteAction" value="Paste" title="Paste after this element">');
    if($('.pasteAction').length==0) {
        ...
        toggleItem('#extend_feeditadvanced_top_menu_paste');
    }
    //showMessage(resources.showMessageCopyElementHeader,resources.showMessageCopyElementText);
}
...
function cutAction(json)
{
    ...
    $('#'+uid.replace(':','\\:')).remove();
    //$('.feEditAdvanced-editButton.cutAction').after('<input type="button" onclick="pasteAction(this);return false;" class="feEditAdvanced-editButton pasteAction" value="Paste" title="Paste after this element">');
    if($('.pasteAction').length==0) {
        $('.feEditAdvanced-editButton.cutAction').each(function(){
            id = $(this).parent().parent().parent().attr('id');
            $(this).after('<input type="button" onclick="pasteAction(\''+id+'\');return false;" class="feEditAdvanced-editButton pasteAction" value="'+resources.editPanelPaste+'" title="'+resources.editPanelPasteTooltip+'">');
    ...
    //showMessage(resources.showMessageCutElementHeader,resources.showMessageCutElementText);
}

function pasteAction(parentUid)
{
    var pasteContent = getCookie('extend_feeditadvanced_copycutitem');
    //console.log(pasteContent);
    var pasteContentArray = pasteContent.split(':');
    var type = pasteContentArray[0];
    var table = pasteContentArray[1];
    var uid = pasteContentArray[2];
    ajax(type+'ContentElement',table,uid,parentUid);

    //var parentuid = json.id;
...
TYPO3.FeEdit.ClipboardObj = Ext.extend(TYPO3.FeEdit.Base, {
	...
	addToClipboard: function(editPanelAction) {
		...
		pasteValues = '<input type="hidden" name="TSFE_EDIT[cmd]" value="' + editPanelAction._getCmd() + '">';
                pasteValues += '<input type="hidden" name="TSFE_EDIT[record]" value="' + rec + '"><input type="hidden" name="TSFE_EDIT[pid]" value="' + editPanelAction.parent.pid + '">';
                pasteValues += '<input type="hidden" name="TSFE_EDIT[flexformPtr]" value="' + editPanelAction.parent.flexformPtr + '">';
                pasteValues += '<input type="hidden" name="TSFE_EDIT[uid]" value="' + thisUID + '">';
		...
		pasteEl += strVal + pasteValues + '</form></div>' + clearBtn + '</div>';
...
TYPO3.FeEdit.EditWindow = Ext.extend(TYPO3.FeEdit.Base, {
	...
	displayIframe: function(headerText, url) {
            headerText = headerText+'<span onclick="closeIframe();return false;" class="fancybox-close"></span><span onclick="loadHelp();return false;" class="help-button"></span>';
