...
class tx_feeditadvanced_editpanel {
	...
	public function init($conf) {
        ...
        function getPid($uid) {
            $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('pid', 'tt_news', 'uid='.intval($uid));
	...
	public function editPanel($content, array $conf, $currentRecord='', array $dataArr=array(), $table='', $allow='', $newUID=0, array $hiddenFields=array()) {
		...
		$this->table = $table;
                ...
                if($table=='tt_news') {
                    if(!$GLOBALS['BE_USER']->isInWebMount($this->getPid($uid))) {
                        return $content;
                    }
                }
		...
		if (($theCmd != 'edit') && ($theCmd != 'new')) {
			...
			if (isset($allow['move']) && $sortField && $GLOBALS['BE_USER']->workspace===0)	{	// Hiding in workspaces because implementation is incomplete
				...
				if ($table != 'pages') {
					$this->panelItems['drag'] 	= '<span class="feEditAdvanced-dragHandle" title="' . $GLOBALS['LANG']->sL('LLL:EXT:extend_feeditadvanced/locallang.xml:dragTitle') . '">&nbsp;</span>';
	...
	protected function editIconLinkWrap($cmd, $name='', $title='', $params='') {
            //echo $GLOBALS['BE_USER']->extGetLL($name);
                //if($GLOBALS['LANG']) $name = $GLOBALS['LANG']->sL('LLL:EXT:extend_feeditadvanced/locallang.xml:'.$name);
		if($GLOBALS['BE_USER']) $name  = strlen($name)  ? $GLOBALS['BE_USER']->extGetLL($name)  : $cmd;
		if($GLOBALS['BE_USER']) $title = strlen($title) ? $GLOBALS['BE_USER']->extGetLL($title) : $name;
                //if($GLOBALS['LANG']) $title = $GLOBALS['LANG']->sL('LLL:EXT:extend_feeditadvanced/locallang.xml:'.$title);
	 ...
	 * @see	editPanel(), editIcons(), t3lib_tsfeBeUserAuth::extEditAction()
	 */
 	protected function wrapContent($markerArray, $table, $uid, array $recordData=array(), array $conf=array(), array $allowOrder=array(), $theCmd='', $newUID='', $fieldList='', $hiddenElement) {
		$editpanelItems = '';
		...
		*/
		...
		if ($recordData['hidden']) {
                    ...
                    if($hiddenElement=='none') {
                        $markerArray['###ALLWRAPPER_CLASS###'] .= ' feEditAdvanced-invisible';
                    }
		}
	...
	protected function formsOnPageForm($markerArray, $table, $dataArr=array(), $currentRecord='', $conf='', $content='', $theCmd='edit', $newUID='', $fieldList='') {
		...
		$buttons .= '<button type="submit" name="TSFE_EDIT[update]" value="1" onclick="' . $updateOnClick . '" class="feEditAdvanced-actionButton saveAction" title="' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:rm.saveDoc', 1) . '"><span>'.$GLOBALS['LANG']->sL('LLL:EXT:extend_feeditadvanced/locallang.xml:saveButton').'</span></button>';
		$buttons .= '<button type="submit" name="TSFE_EDIT[update_close]" value="1" onclick="' . $updateCloseOnClick . '" class="feEditAdvanced-actionButton saveCloseAction" title="' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:rm.saveCloseDoc', 1) . '"><span>'.$GLOBALS['LANG']->sL('LLL:EXT:extend_feeditadvanced/locallang.xml:saveCloseButton').'</span></button>';
		$buttons .= '<button type="submit" name="TSFE_EDIT[close]" value="1" onclick="' . $closeOnClick . '" class="feEditAdvanced-actionButton closeAction" title="' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:rm.closeDoc', 1) . '"><span>'.$GLOBALS['LANG']->sL('LLL:EXT:extend_feeditadvanced/locallang.xml:closeButton').'</span></button>';
		$buttons .= '</div>';
	...
	*/
	protected function addFormIncludes($tceforms=0) {
		...
		$pageRenderer = $GLOBALS['TSFE']->getPageRenderer();
                ...
                $pageRenderer->addMetaTag('<meta http-equiv="X-UA-Compatible" content="IE=9" />');
		...
		$incJS .= '<script type="text/javascript">
					...
					if (advFields.length) {
						...
						$(\'simpleadv_toggle\').blur();
                                ...
                                Ext.onReady(function() {
                                    ...
                                    Ext.select("fieldset.t3-form-palette-fieldset").each(function(fieldset) {
                                        i=0;
                                        fieldset.select("span.t3-form-palette-field-container").each(function(containerElement) {
                                            if(i===3 || (i===2 && this.getX()>900)) {
                                                Ext.DomHelper.insertBefore(this, {
                                                    html: "<br/>",
                                                }, true);
					    ...
					    items[i].style.borderLeft = \'none\';
                                            ...
                                            if(marginLeft < 600) {
                                                items[i].style.marginLeft = marginLeft+\'px\';
                                                items[i].style.width = \'700px\';
                                            } else {
                                                items[firstI].style.width = \'700px\';
                                                items[i].style.marginLeft = \'700px\';
                                            }
                                        } /*else if(items[i].className === "c-tablayer" && firstHeight==null) {
                                            firstHeight = items[i].offsetHeight;
                                        }*/
                                    }
                                    
                                    items[otherOptionsId].parentNode.removeChild(items[otherOptionsId]);
                                    /*if(firstHeight < heightSum) {
                                        items[firstI].style.height = heightSum+\'px\';
                                        Ext.get("formsOnPage").parent().setHeight(heightSum,false);
                                    } else {
                                        Ext.get("formsOnPage").parent().setHeight(firstHeight,false);
                                    }*/
                                    
                                    document.getElementsByTagName("head")[0].removeChild(document.getElementsByTagName("head")[0].children[0]);
                ...
                $incJS .= '<style>
                            ...
                            table.typo3-TCEforms select.formField, table.typo3-TCEforms span.filetypes, .t3-form-palette-field-container select.select {
                                width:200px;
                            }
                            
                            .x-tool {
                            background-image: url(../../typo3/sysext/t3skin/extjs/images/panel/tool-sprites.gif) !important;
                            }
                            ...
                            table.typo3-TCEforms textarea.formField {
